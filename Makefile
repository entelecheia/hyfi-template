...
mkvirtualenv: ## create the project environment
	@python3 -m venv "$$WORKON_HOME/hyfi-template"
	@. "$$WORKON_HOME/hyfi-template/bin/activate"
	@pip install --upgrade pip setuptools wheel

mkvirtualenv-system: ## create the project environment with system site packages
	@python3 -m venv "$$WORKON_HOME/hyfi-template" --system-site-packages
	@. "$$WORKON_HOME/hyfi-template/bin/activate"
	@pip install --upgrade pip setuptools wheel
...
init-project: initialize ## initialize the project (Warning: do this only once!)
	@copier copy --trust --data 'code_template_source=gh:entelecheia/hyfi-template' --answers-file .copier-config.yaml --vcs-ref=HEAD gh:entelecheia/hyperfast-python-template .

reinit-project: install-copier ## reinitialize the project (Warning: this may overwrite existing files!)
	@bash -c 'args=(); while IFS= read -r file; do args+=("--skip" "$$file"); done < .copierignore; copier copy --trust "$${args[@]}" --data 'code_template_source=gh:entelecheia/hyfi-template' --answers-file .copier-config.yaml --vcs-ref=HEAD gh:entelecheia/hyperfast-python-template .'
